...
def _convert_with_markitdown(file_path: Path) -> str:
    ...
    logger.info(f"[File Conversion] Converting with MarkItDown: {file_path.name}")
    try:
        ...
        logger.info(f"[File Conversion] MarkItDown conversion successful: {file_path.name} ({len(text)} chars)")
        return text
    except Exception as e:
        logger.exception(f"[File Conversion] MarkItDown failed for {file_path.name}: {e}")
        raise
...
def _do_convert(file_path: Path, pdf_converter: str) -> str:
    ...
    """
    logger.info(f"[File Conversion] Starting conversion: {file_path.name} (type: {file_path.suffix}, converter: {pdf_converter})")
    is_pdf = file_path.suffix.lower() == ".pdf"

    if is_pdf and pdf_converter != "markitdown":
        logger.debug(f"[File Conversion] Trying pymupdf4llm first for PDF: {file_path.name}")
        ...
        if pymupdf_text is not None:
            ...
            if pdf_converter == "pymupdf4llm":
                ...
                logger.info(f"[File Conversion] Using pymupdf4llm result for {file_path.name}")
                return pymupdf_text
            ...
            if not _pymupdf_output_too_sparse(pymupdf_text, file_path):
                logger.info(f"[File Conversion] pymupdf4llm result looks good for {file_path.name}")
                return pymupdf_text
            logger.warning(
            ...
            logger.debug(f"[File Conversion] pymupdf4llm not available, using MarkItDown for {file_path.name}")
