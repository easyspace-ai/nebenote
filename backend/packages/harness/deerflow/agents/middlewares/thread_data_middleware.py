...
class ThreadDataMiddleware(AgentMiddleware[ThreadDataMiddlewareState]):
    ...
    If the thread belongs to a notebook, the uploads path points to the
    notebook's unified uploads directory instead.
    ...
    def _get_thread_paths(self, thread_id: str) -> dict[str, str]:
        ...
        If the thread belongs to a notebook, uses the notebook's unified
        user-data directory (workspace, uploads, outputs all shared).
        """
        ...
        if notebook:
            # Use notebook's unified user-data directory for everything
            nb_paths = nb_manager.paths
            # Ensure all notebook directories exist
            nb_paths.ensure_notebook_dirs(notebook.notebook_id)

            return {
                "workspace_path": str(nb_paths.workspace_dir(notebook.notebook_id)),
                "uploads_path": str(nb_paths.uploads_dir(notebook.notebook_id)),
                "outputs_path": str(nb_paths.outputs_dir(notebook.notebook_id)),
                "notebook_id": notebook.notebook_id,
            }

        # Regular thread paths
        return {
