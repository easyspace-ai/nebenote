...
from deerflow.notebook.paths import get_notebook_paths
...
_NOTEBOOK_VIRTUAL_PATH = "/mnt/notebook"
...
def _is_notebook_path(path: str) -> bool:
    """Check if a path is under the notebook virtual path."""
    return path == _NOTEBOOK_VIRTUAL_PATH or path.startswith(f"{_NOTEBOOK_VIRTUAL_PATH}/")


def _resolve_notebook_path(path: str) -> str:
    """Resolve a notebook virtual path to actual host path."""
    return replace_virtual_path(path, None)
...
def _resolve_local_read_path(path: str, thread_data: ThreadDataState) -> str:
    ...
    if _is_notebook_path(path):
        return _resolve_notebook_path(path)
...
def replace_virtual_path(path: str, thread_data: ThreadDataState | None) -> str:
    ...
    Mapping:
        ...
        /mnt/notebook/{notebook_id}/* -> notebooks/{notebook_id}/documents/*
    ...
    """
    # Handle notebook paths: /mnt/notebook/{notebook_id}/*
    if path == _NOTEBOOK_VIRTUAL_PATH or path.startswith(f"{_NOTEBOOK_VIRTUAL_PATH}/"):
        nb_paths = get_notebook_paths()
        if path == _NOTEBOOK_VIRTUAL_PATH:
            return str(nb_paths.notebooks_dir)
        # Extract notebook_id and rest of path
        rest = path[len(_NOTEBOOK_VIRTUAL_PATH) :].lstrip("/")
        parts = rest.split("/", 1)
        if parts:
            notebook_id = parts[0]
            sub_rest = parts[1] if len(parts) > 1 else ""
            # Map /mnt/notebook/{notebook_id}/... to notebooks/{notebook_id}/...
            # This gives access to the entire notebook directory
            nb_dir = nb_paths.notebook_dir(notebook_id)
            if sub_rest:
                result = _join_path_preserving_style(str(nb_dir), sub_rest)
            else:
                result = str(nb_dir)
            if path.endswith("/") and not result.endswith(("/", "\\")):
                result += _path_separator_for_style(result)
            return result
...
def validate_local_tool_path(path: str, thread_data: ThreadDataState | None, *, read_only: bool = False) -> None:
    ...
    # Notebook paths — read-only access only
    if _is_notebook_path(path):
        if not read_only:
            raise PermissionError(f"Write access to notebook path is not allowed: {path}")
        return
...
def validate_local_bash_command_paths(command: str, thread_data: ThreadDataState | None) -> None:
    ...
    for absolute_path in _ABSOLUTE_PATH_PATTERN.findall(command):
        ...
        # Allow notebook path (path-traversal check only)
        if absolute_path == _NOTEBOOK_VIRTUAL_PATH or absolute_path.startswith(f"{_NOTEBOOK_VIRTUAL_PATH}/"):
            _reject_path_traversal(absolute_path)
            continue
...
def ls_tool(runtime: ToolRuntime[ContextT, ThreadState], description: str, path: str) -> str:
    ...
    try:
        ...
        if is_local_sandbox(runtime):
            ...
            elif _is_notebook_path(path):
                path = _resolve_notebook_path(path)
