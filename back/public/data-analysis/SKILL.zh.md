---
name: data-analysis
description: 当用户上传 Excel（.xlsx/.xls）或 CSV 文件并想要执行数据分析、生成统计、创建摘要、数据透视表、SQL 查询或任何形式的结构化数据探索时使用此技能。支持多工作表 Excel 工作簿、聚合、过滤、连接和将结果导出为 CSV/JSON/Markdown。
---
...
本技能使用 DuckDB（一个进程内分析 SQL 引擎）分析用户上传的 Excel/CSV 文件。它支持模式检查、基于 SQL 的查询、统计摘要和结果导出，所有这些都通过单个 Python 脚本完成。
...
对于每个数值列返回：count、mean、std、min、25%、50%、75%、max、null_count。
...
- **Excel 文件**：每个工作表成为一个表，以工作表命名（例如 `Sheet1`、`Sales`、`Revenue`）
- **CSV 文件**：表名是去掉扩展名的文件名（例如 `data.csv` → `data`）
- **多个文件**：所有文件中的所有表都在同一查询上下文中可用，支持跨文件连接
...
用户上传 `sales_2024.xlsx`（工作表：`Orders`、`Products`、`Customers`）并询问："分析我的销售数据 — 显示按收入排名的产品及月度趋势。"
...
- 具有相同文件的后续调用将直接使用缓存的数据库（接近即时的启动）
- 缓存是透明的 — 无需额外参数

这在针对相同数据文件运行多个查询时特别有用（inspect → query → summary）。

## 注意事项

- DuckDB 支持完整 SQL，包括窗口函数、CTE、子查询和高级聚合
- Excel 日期列自动解析；使用 DuckDB 日期函数（`DATE_TRUNC`、`EXTRACT` 等）
- 对于非常大的文件（100MB+），DuckDB 高效处理它们，不会将所有内容加载到内存中
