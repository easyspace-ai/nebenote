...
description: 当用户请求生成、创建或想象视频时使用此技能。支持结构化提示和参考图像以指导生成。
---
...
本技能使用结构化提示和 Python 脚本生成高质量视频。工作流程包括创建 JSON 格式提示并执行视频生成，可选择使用参考图像。

## 核心能力

- 为 AIGC 视频生成创建结构化 JSON 提示
- 支持参考图像作为指导或视频的第一/最后一帧
...
用户请求："生成一段短视频，描绘《纳尼亚传奇：狮子、女巫和魔衣橱》的开场场景"
...
```json
{
  "title": "纳尼亚传奇 - 火车站告别",
  "background": {
    "description": "二战期间在拥挤的伦敦火车站的疏散场景。蒸汽和烟雾弥漫空中，孩子们被送往乡村以逃离闪电战。",
    "era": "1940 年代战时英国",
    "location": "伦敦火车站站台"
  },
...
步骤 3：使用 image-generation 技能生成视频的参考图像

加载 image-generation 技能并根据技能生成单个参考图像 `narnia-farewell-scene-01.jpg`。
...
- 使用 present_files 工具与用户分享生成的视频（首先），以及生成的图像（如适用）
- 提供生成结果的简要描述
- 如需调整，提供迭代选项
