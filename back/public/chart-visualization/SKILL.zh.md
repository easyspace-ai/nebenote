...
本技能提供将数据转换为可视化图表的完整工作流程。它处理图表选择、参数提取和图像生成。
...
- **时间序列**：使用 `generate_line_chart`（趋势）或 `generate_area_chart`（累积趋势）。对于两种不同比例尺使用 `generate_dual_axes_chart`。
...
选择图表类型后，读取 `references/` 目录中的相应文件（例如 `references/generate_line_chart.md`）以识别必填和可选字段。
从用户输入中提取数据并映射到预期的 `args` 格式。

### 3. 图表生成

使用 JSON 负载调用 `scripts/generate.js` 脚本。
...
每个图表类型的详细规格位于 `references/` 目录。查阅这些文件以确保传递给脚本的 `args` 匹配预期的模式。

## 许可证

本 `SKILL.md` 由 [antviz/chart-visualization-skills](https://github.com/antvis/chart-visualization-skills) 提供。
