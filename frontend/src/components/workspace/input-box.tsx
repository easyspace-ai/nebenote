...
import { NotebookDocumentMention } from "./notebook-document-mention";
...
export function InputBox({
  ...
  const handleSelectDocument = useCallback(
    (doc: Document, _notebookId: string) => {
      // Format: @[Document Title](doc_abc123)
      const mention = `@[${doc.title}](${doc.doc_id}) `;
      const current = textInput.value ?? "";
      textInput.setInput(current + mention);
    },
    [textInput]
  );
