...
const config = {
  ...
  async rewrites() {
    ...
    if (!process.env.NEXT_PUBLIC_BACKEND_BASE_URL) {
      ...
      rewrites.push({
        source: "/api/notebooks",
        destination: `${gatewayURL}/api/notebooks`,
      });
      rewrites.push({
        source: "/api/notebooks/:path*",
        destination: `${gatewayURL}/api/notebooks/:path*`,
      });
      rewrites.push({
        source: "/api/models",
      ...
      rewrites.push({
        ...
        destination: `${gatewayURL}/api/artifacts`,
      });
      rewrites.push({
        source: "/api/artifacts/:path*",
        destination: `${gatewayURL}/api/artifacts/:path*`,
      });
      rewrites.push({
        source: "/api/suggestions",
        destination: `${gatewayURL}/api/suggestions`,
      });
      rewrites.push({
        source: "/api/suggestions/:path*",
        destination: `${gatewayURL}/api/suggestions/:path*`,
      });
    }
