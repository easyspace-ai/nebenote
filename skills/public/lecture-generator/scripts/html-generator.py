...
try:
    ...
    # 运行时自动 pip 会在沙箱/离线环境卡住或拖长整轮 Agent，改为快速失败并提示预装依赖
    sys.stderr.write(
        "错误: 未安装 jinja2 / markdown。\n"
        "请在运行环境中执行: pip install 'jinja2>=3.1' 'markdown>=3.4'\n"
        "或: pip install -r skills/pubic/lecture-generator/requirements.txt\n",
    )
    sys.exit(1)
